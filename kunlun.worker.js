...
async function handlePostStatus(request, env, url) {
    ...
    try {
        ...
        if (results && results[0] && results[0].id) {
            ...
            // 查不到就插入新记录
            const { meta: countMeta, results: countResults } = await env.DB
                .prepare('SELECT COUNT(*) AS count FROM client')
                .run();
            rows_read += countMeta.rows_read;
            rows_written += countMeta.rows_written;
            console.log('results of count of client:', countResults);

            // 插入新记录
            clientId = countResults[0].count + 1;
            const { meta: insertMeta } = await env.DB
                .prepare('INSERT INTO client (id, machine_id, name) VALUES (?,?,?);')
                .bind(clientId, machineId, 'node')
                .run();
            rows_read += insertMeta.rows_read;
            rows_written += insertMeta.rows_written;
        }
    } catch (error) {
